...
# ============================================================================
# Parallel Launcher (runs only in parent process)
# ============================================================================
run_parallel_benchmarks() {
    local raw_ids="$1"
    raw_ids=$(echo "$raw_ids" | tr ' ' ',' | tr -s ',')

    IFS=',' read -r -a MACHINE_IDS <<< "$raw_ids"
    ...
    for mid in "${MACHINE_IDS[@]}"; do
        [[ -z "$mid" ]] && continue
        if [[ ! "$mid" =~ ^[0-9]+$ ]]; then
            log_error "Invalid machine ID '$mid' (must be numeric)"
            exit 1
        fi
        VALID_IDS+=("$mid")
    done
    ...
    if [[ ${#VALID_IDS[@]} -eq 0 ]]; then
        log_error "No valid machine IDs provided"
        exit 1
    fi

    # If just one ID, fall through to normal flow
    if [[ ${#VALID_IDS[@]} -eq 1 ]]; then
        MACHINE_ID="${VALID_IDS[0]}"
        RUN_QUICKMARK_CHILD=1
        return
    fi

    log_step "Parallel Mode: Launching ${#VALID_IDS[@]} benchmarks"
    ...
    for idx in "${!VALID_IDS[@]}"; do
        local mid="${VALID_IDS[$idx]}"
        local log_file="${SCRIPT_DIR}/logs/quickmark_${mid}_$(date +%Y%m%d-%H%M%S).log"
        LOG_PATHS[$idx]="$log_file"
        STATUSES[$idx]="starting"
        DONE[$idx]=0

        log_info "Starting machine $mid (log: $log_file)"
        (RUN_QUICKMARK_CHILD=1 "$0" "$mid" >"$log_file" 2>&1 &)
        PIDS[$idx]=$!
    done

    log_step "Monitoring Benchmarks"
    log_info "Tail a specific run with: tail -f logs/quickmark_<ID>_*.log"
    log_info "Press Ctrl+C to stop monitoring (jobs keep running in background)"

    while :; do
        local running=0
        printf "\n%-12s %-8s %-18s %s\n" "Machine ID" "PID" "Status" "Log"
        for idx in "${!VALID_IDS[@]}"; do
            local mid="${VALID_IDS[$idx]}"
            local pid="${PIDS[$idx]}"
            local log_file="${LOG_PATHS[$idx]}"
            local status="${STATUSES[$idx]}"

            if [[ ${DONE[$idx]} -eq 0 ]]; then
    ...
    log_success "All parallel runs completed"
    exit 0
}

# If this is the parent process (no child flag), parse CLI for multi-run
if [[ -z "${RUN_QUICKMARK_CHILD:-}" ]]; then
    if [[ $# -ge 1 ]]; then
        INPUT_IDS="$1"
    else
        echo -e "${YELLOW}Enter one or more Vast.ai machine IDs (comma-separated):${NC}"
        read -r INPUT_IDS
    fi
    run_parallel_benchmarks "$INPUT_IDS"
fi
